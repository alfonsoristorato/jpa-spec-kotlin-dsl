...
class CollectionTest(
    ...
    private val organisationRepository: OrganisationRepository,
        ...
        context("collectionContains for PredicateSpecification tests whether an element is contained in a native collection column") {
        ...
        context(
            "collectionNotContains for PredicateSpecification tests whether an element is not contained in a native collection column",
        ) {
        ...
        context(
            "collectionIntersects for PredicateSpecification tests whether a native collection column shares at least one element with a sub-collection",
        ) {
            expect("returns organisations whose collection shares at least one element") {
                val spec = Organisation::identifiers.collectionIntersects(setOf("identifier1", "nonexistent"))
                val result = organisationRepository.findAll(spec)
                result shouldHaveSize 1
                result[0].name shouldBe "Org With Identifiers"
            }
            expect("returns empty when no elements are shared") {
                val spec = Organisation::identifiers.collectionIntersects(setOf("nonexistent"))
                val result = organisationRepository.findAll(spec)
                result shouldHaveSize 0
            }
            expect("with nested types") {
        ...
        context(
            "collectionNotIntersects for PredicateSpecification tests whether a native collection column shares no element with a sub-collection",
        ) {
            expect("returns organisations whose collection shares no element") {
                val spec = Organisation::identifiers.collectionNotIntersects(setOf("identifier1", "identifier2"))
                val result = organisationRepository.findAll(spec)
                result shouldHaveSize 1
                result[0].name shouldBe "Org Without Identifiers"
            }
            expect("returns all organisations when no collection shares any element") {
                val spec = Organisation::identifiers.collectionNotIntersects(setOf("nonexistent"))
                val result = organisationRepository.findAll(spec)
                result shouldHaveSize 2
            }
            expect("with nested types") {
