...
class ArrayTest(
    ...
    private val organisationRepository: OrganisationRepository,
        ...
        context("arrayIntersects for Predicate tests whether a native array column shares at least one element with a sub-array") {
            expect("returns organisations whose array shares at least one element") {
                val result =
                    organisationRepository.findAll { root, _, cb ->
                        Organisation::tags.arrayIntersects(root, cb, arrayOf("tag1", "nonexistent"))
                    }
                result shouldHaveSize 1
                result[0].name shouldBe "Org With Tags"
            }
            expect("returns empty when no elements are shared") {
                val result =
                    organisationRepository.findAll { root, _, cb ->
                        Organisation::tags.arrayIntersects(root, cb, arrayOf("nonexistent"))
                    }
                result shouldHaveSize 0
            }
            expect("with nested types") {
                val result =
                    personaRepository.findAll { root, _, cb ->
                        (Persona::organisation / Organisation::tags).arrayIntersects(root, cb, arrayOf("tag1", "nonexistent"))
                    }
                result shouldHaveSize 1
                result[0].name shouldBe "Persona 1"
            }
        ...
        context("arrayNotIntersects for Predicate tests whether a native array column shares no element with a sub-array") {
            expect("returns organisations whose array shares no element") {
                val result =
                    organisationRepository.findAll { root, _, cb ->
                        Organisation::tags.arrayNotIntersects(root, cb, arrayOf("tag1", "tag2"))
                    }
                result shouldHaveSize 1
                result[0].name shouldBe "Org Without Tags"
            }
            expect("returns all organisations when no array shares any element") {
                val result =
                    organisationRepository.findAll { root, _, cb ->
                        Organisation::tags.arrayNotIntersects(root, cb, arrayOf("nonexistent"))
                    }
                result shouldHaveSize 2
            }
            expect("with nested types") {
                val result =
                    personaRepository.findAll { root, _, cb ->
                        (Persona::organisation / Organisation::tags).arrayNotIntersects(root, cb, arrayOf("tag1", "nonexistent"))
                    }
                result shouldHaveSize 1
                result[0].name shouldBe "Persona 2"
            }
