...
import io.github.alfonsoristorato.jpaspeckotlindsl.specification.combiner.and
    ...
    private val organisationRepository: OrganisationRepository,
                ...
                TestFixtures.createOrganisation(
                    ...
                    organisationInfo =
                        TestFixtures.createOrganisationInfo(
                            addressInfo =
                                TestFixtures.createAddressInfo(
                                    street = "Street 1",
                                    city = "City 1",
                                ),
                            contactInfo = null,
        ...
        context("equal for Specification checks for equality") {
            ...
            expect("with nested types") {
                val organisationWithStreet =
                    (Organisation::organisationInfo / OrganisationInfo::addressInfo / AddressInfo::street)
                        .equal("Street 2")
                val organisationWithContact =
                    (Organisation::organisationInfo / OrganisationInfo::contactInfo / ContactInfo::email)
                        .equal("Email 2")
                val result =
                    organisationRepository.findAll(organisationWithStreet and organisationWithContact)
                result shouldHaveSize 1
                result.first().name shouldBe "organisation 2"
            }
        ...
        context("notEqual for Specification checks for inequality") {
            expect("with nested types") {
                val organisationNotStreet1 =
                    (Organisation::organisationInfo / OrganisationInfo::addressInfo / AddressInfo::street)
                        .notEqual("Street 1")
                val result =
                    organisationRepository.findAll(organisationNotStreet1)
                result shouldHaveSize 1
                result.first().name shouldBe "organisation 2"
            }
            expect("with declared types") {
