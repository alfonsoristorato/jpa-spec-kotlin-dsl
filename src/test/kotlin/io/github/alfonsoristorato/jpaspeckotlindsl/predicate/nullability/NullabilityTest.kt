...
class NullabilityTest(
    ...
    private val organisationRepository: OrganisationRepository,
        ...
        context("isNull for Predicate checks for property to be null") {
            expect("with non-nullable property") {
                val result =
                    personaRepository.findAll { root, _, cb ->
                        Persona::firstLogin.isNull(root, cb)
                    }
            ...
            expect("with nullable property - isMagic") {
                val result =
                    personaRepository.findAll { root, _, cb ->
                        Persona::isMagic.isNull(root, cb)
                    }
            ...
            expect("with nullable property - lastName") {
                val result =
                    personaRepository.findAll { root, _, cb ->
                        Persona::lastName.isNull(root, cb)
                    }
                result shouldHaveSize 1
                result[0].name shouldBe "Persona 3"
            }
        }
        context("isNotNull for Predicate checks for property to not be null") {
            expect("with non-nullable property") {
                val result =
                    personaRepository.findAll { root, _, cb ->
                        Persona::firstLogin.isNotNull(root, cb)
                    }
            ...
            expect("with nullable property - isMagic") {
                val result =
                    personaRepository.findAll { root, _, cb ->
                        Persona::isMagic.isNotNull(root, cb)
                    }
            ...
            expect("with nullable property - lastName") {
                val result =
                    personaRepository.findAll { root, _, cb ->
                        Persona::lastName.isNotNull(root, cb)
                    }
                result shouldHaveSize 2
                result.map { it.name }.toSet() shouldBe setOf("Persona 1", "Persona 2")
            }
        }
        context("isNull for Predicate checks for nested property to be null") {
        ...
        context("isNotNull for Predicate checks for nested property to not be null") {
            expect("with nested nullable types") {
                val result =
                    organisationRepository.findAll { root, _, cb ->
                        (Organisation::organisationInfo / OrganisationInfo::contactInfo / ContactInfo::email)
